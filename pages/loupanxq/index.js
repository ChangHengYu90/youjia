...
Page({
  ...
  getloupan(){
   var that = this
    ...
    util.requestLoading(that.data.apis + '/api/hourse/hoursedetail.aspx', foptions, data, '', function (res) {
      console.log(res)
      if (res.code == '200') {
        let shuju = res.data
  ...
  gollokyj(){
    ...
    wx.navigateTo({
      url: '../yjgz/index?gz=' + shuju.rateIntroduce + '&phone=' + shuju.leadingOfficialMobile + '&rate=' + encodeURIComponent(shuju.rate)
    })
  },
  golookqk(){
    var that = this
    let shuju = that.data.loupan
    wx.navigateTo({
      url: '../qkgz/index?gz=' + shuju.accurateCustomer + '&phone=' + shuju.leadingOfficialMobile
    })
  },
  bdphone(){
    var that = this
  ...
  gomorelp(){
    var that = this
      wx.navigateTo({
        url: '../loupanxx/index?id='+that.data.id,
      })
  },
  goloupandt(e){
    var that = this
    ...
    if (num==0){
      ...
      wx.navigateTo({
        url: '../loupandt/index?id=' + that.data.id,
      })
    }
  },
  gotctj(e){
