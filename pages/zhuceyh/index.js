...
Page({
  ...
  hqjgyqm(e){
    var that = this
    that.setData({
      jgyqm: e.detail.value
    })
  },
  hqdljjr(e){
    var that = this
    that.setData({
      dlyqm: e.detail.value
    })
  },
  huoquyzm() {
    var that = this
  ...
  submit(){
    ...
    let jgyqm = that.data.jgyqm
    let dlyqm = that.data.dlyqm
    var phoneRexp = /^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/;
    if (phone == '' || phone == undefined || phone==null){
      wx.showToast({
        title: '请输入手机号',
    ...
    if (jgyqm == '' || jgyqm == undefined || jgyqm == null) {
      wx.showToast({
        title: '请输入邀请码',
        icon: 'none',
        duration: 1500,
    ...
    if (jgyqm!=''){
      foptions.push('inviteCode' + jgyqm)
      data.inviteCode = jgyqm
    }
    ...
    if (sfzh != '' && sfzh != undefined && sfzh != null) {
      foptions.push('cardNo' + sfzh)
      data.cardNo = sfzh
    }
    util.requestLoading(that.data.apis + '/api/user/broker_register.aspx', foptions, data, '', function (res) {
      console.log(res)
      wx.hideLoading()
      if (res.code == '200') {
        wx.showToast({
  ...
  lookxy() {
    wx.navigateTo({
      url: '../wangye/index?url=' + encodeURIComponent('https://youjiahouse.cn/api/useragreenment.aspx'),
    })
  }
})
